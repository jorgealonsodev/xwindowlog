...
impl Tracker {
    ...
    /// X11 capture never actually stops while paused (only this tracker discards what it
    /// reports), so the very next real `ActiveWindow` event promptly resolves `unknown` back
    /// to `active`, the same reasoning `locked -> unknown` already relies on. Every other
...
mod tests {
    ...
    // --- 15.11: paused + Resume/PauseExpiry -> unknown (interval-tracking "resume ends an
    // active pause"). The next real ActiveWindow event, already generated by x11.rs regardless
    // of pause (capture never actually stops — only the tracker discards it while paused),
    // promptly resolves `unknown` back to `active`, exactly like `locked -> unknown` already
    // does on unlock. ---------------------------------------------------------------------

    #[test]
    fn paused_plus_resume_transitions_to_unknown() {
        ...
        send(&mut tracker, SourceEvent::Pause { until: None }, &clock);

        clock.advance(Duration::from_secs(4));
        let effects = send(&mut tracker, SourceEvent::Resume, &clock);
    ...
    #[test]
    fn paused_plus_pause_expiry_deadline_transitions_to_unknown_automatically() {
