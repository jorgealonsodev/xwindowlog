...
- **RF-27** *(new)* The daemon takes a `delay` inhibitor of type `sleep` at startup. On receiving `PrepareForSleep(true)`: it closes the current interval, forces the write buffer to be flushed to disk and releases the descriptor, allowing the suspend to proceed. On receiving `PrepareForSleep(false)`: it opens an `unknown` interval and takes the inhibitor again. If `Inhibit()` fails (restrictive polkit policies), it degrades with a warning and continues with a best-effort guarantee, without blocking startup.
...
- **RF-33** *(new)* Handling of `SIGTERM` and `SIGINT`: closes the open interval with `end = now()` in whatever state it was, forces the buffer flush, releases the lock file and the logind inhibitor, and exits with code 0. Without this, metric M-1 breaks every time the user logs out or stops the service.
...
- **RF-12** *(corrected)* Write policy, which v1.2 left as "batch every 30 s" without specifying what is lost:
  1. **Opening an interval is written immediately**, in its own transaction, with `"end" = NULL`. It is cheap (it happens only on window or state changes, not every second) and it makes the current interval always reconstructible from disk.
  2. **Closing is grouped into the 30 s batch**, except closes caused by `afk`, `locked` or `paused`, which are written immediately.
  3. Worst case on power loss: the most recent interval is left with `end = NULL` even though it had already been closed. That is **recoverable** (RF-36). Losing the opening entirely would not be: it would leave a gap with no record that it ever existed.
  > *Verified:* in WAL mode with `synchronous = NORMAL`, SQLite guarantees that the database **is never corrupted** on power loss, but it **does not guarantee that the last committed transactions survive**. This qualifies RNF-6: consistency is assured, durability of the last few seconds is not. It is an accepted trade-off, not an oversight.
- **RF-13** *(corrected)* `xwindowlog prune --older-than 180d` for optional retention. It must: delete `intervals` with `"end" < cutoff`, **never** the open interval; delete orphan rows from `apps`/`titles` except the sentinels; **not** delete `rules` or `projects` even if their original history is gone, because a confirmed rule still applies to future intervals; and run `VACUUM` at the end, since without it a `DELETE` does not shrink the file, it only marks pages as reusable, and "retention" means to the user that the file shrinks. `prune` is a separate CLI command, never run from the daemon while it is live, so as not to compete with the write buffer.
...
├── store.rs     — rusqlite: migrations, write buffer, aggregated queries
...
x11.rs (reads the real title)
  → exclude.rs (evaluates rules; replaces with [hidden] or redacts secrets)
    → tracker.rs (builds the interval, already sanitized)
      → store.rs (write buffer)
        → status / today / export / mcp.rs (always read from the store, never from X11)
```

The sanitization point is **before the write buffer** and therefore before every read path. It is stated explicitly so that it does not get broken by adding, for example, a debug log that prints the raw title.
...
- It deletes nothing by default unless you enable `retention_days` or run `prune`/`forget`.
...
- **D-3 — Default retention.** Should `retention_days = 365` be active by default (data minimization) or `0` by default (the user decides)? This document proposes 365 with automatic pruning in v2.
...
|---|---|---|
| Phase 1 | RF-1 to RF-13, RF-20 to RF-36, RF-47 to RF-54, RF-60 to RF-63; RNF-1 to RNF-6, RNF-11 | Unit tests, Xvfb+WM integration, properties P1-P3, benchmarks |
| Phase 2 | RF-14 to RF-18, RF-38 to RF-40, RF-46, RF-55 to RF-58; RNF-7 to RNF-10, RNF-12 | JSON-RPC golden files, in-memory SQLite, token measurement |
| Phase 3 | RF-15 (rules), RF-17, RF-41, RF-45, RF-59, RF-64 | Property P4, rule overlap, token cycle |
