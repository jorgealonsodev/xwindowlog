...
- **Cargo.toml:** `edition = "2021"`, release profile with `lto = true`, `codegen-units = 1`, `strip = true`, and `panic = "unwind"`. The shared profile preserves panic isolation for the long-lived MCP process (see RNF-9 and resolved decision D-2).
...
- **RF-24** *(new; mechanism corrected on 2026-09-17)* EWMH verification at startup. `_NET_ACTIVE_WINDOW` is maintained by the window manager, **not by the X11 server**. The daemon therefore verifies the window manager's own claim, on the window manager's own terms:
  1. It reads the **`_NET_SUPPORTED` property of the root window** (type `ATOM[]`) and confirms that `_NET_ACTIVE_WINDOW` is listed among its atoms. This property is how EWMH §2 specifies a window manager advertises the hints it implements; a WM that is not running has not written it.
  2. It performs a **`_NET_SUPPORTING_WM_CHECK` liveness probe** (EWMH §2): it reads that property from the root window, which names a child window the window manager created, and confirms that the same property on that child window points back at the child itself. A window manager that has exited takes that window with it, so the probe fails with `BadWindow` — which is what distinguishes a live compliant WM from the stale `_NET_SUPPORTED` a dead one leaves behind on the server-owned root window.

  If either step fails, the daemon emits an explicit diagnostic on stderr stating that the window manager does not appear to be EWMH-compliant, and degrades to `GetInputFocus` as an approximation. **It must never sit silently waiting for events that will never arrive**: that silent failure would hit precisely the tiling window manager audience, the most likely to try this project.

  > *(Mechanism corrected 2026-09-17, during Phase 9 implementation, after an adversarial verification pass.)* The original wording specified `intern_atom(only_if_exists = true)` on `_NET_SUPPORTED` and `_NET_ACTIVE_WINDOW`. That check does not test what the requirement means, and the implementation that faithfully obeyed it was therefore structurally wrong. **Interning an atom queries the X server's global atom-**name** table**, which is server-wide, shared by every client, created on first use by *anybody*, and outlives the client that created it. It records that a string has been assigned a numeric id. It records nothing about whether a window manager is running, has ever run, or implements the hint. Proven under Xvfb: an ordinary client that merely interns both names — GTK3 interns `_NET_ACTIVE_WINDOW` unconditionally at startup, even with no WM present — makes the check report "compliant" on a display with **no window manager at all**, after which the daemon subscribes and blocks forever; and after a compliant WM exits, the names remain interned and the check still reports "compliant". Both are exactly the silent failure this requirement exists to prevent, on exactly the audience it names. Aggravating detail from the same pass: the daemon's own atom resolution ran *before* the check and interned `_NET_ACTIVE_WINDOW` with `only_if_exists = false`, so the process manufactured half of the evidence it then inspected. The requirement's intent and its reason are unchanged; only the mechanism is corrected, from a server-global name lookup to the two window-manager-owned root properties that actually carry the claim.
...
- **RF-65** *(new, 2026-09-17)* Degradation chain when **logind itself is unreachable**, not merely when one call is refused. RF-25 gives X11 absence detection a three-step chain; D-Bus had no counterpart, so a session bus that is absent at startup — containers, minimal or non-systemd systems, a broken user bus — was undefined behaviour. If the bus cannot be reached at startup, the daemon logs a warning **once**, disables lock and suspend detection, and continues on X11 signals alone; `locked` intervals simply never occur. It retries the connection with the RF-32 backoff, and resumes full detection if the bus appears later. Missing logind MUST NOT block startup, and MUST NOT be reported as an error exit: the daemon still delivers its primary function. This is distinct from RF-27's narrower case, where the bus is present but `Inhibit()` is refused by polkit.
...
- **RF-66** *(new, 2026-09-17)* **Interval clipping.** Every aggregation over a date range MUST operate on intervals clipped to that range, never on raw intervals, and MUST NOT count any second twice or drop any second that falls inside the range. This subsection already described the mechanism and the PRD already stated that metric M-1 is unreachable without it, but it carried no requirement identifier, so the single most load-bearing mechanism in the document was untraceable and could not be verified against a spec. It is numbered here. It is closed in Phase 1, because `today` depends on it, and Phase 2's `summary` and Phase 3's `export` build on the same clipped CTE.
...
- **RF-49** *(new)* `xwindowlog pause [--minutes N]` / `resume`. During the pause no `active` intervals are opened; a `paused` interval is recorded so that the working day still adds up. `status` must show visibly that it is paused: a pause the user cannot verify at a glance is useless. `--minutes` avoids the classic incognito-mode failure of being left on and forgotten. *(Added 2026-09-17.)* **Control channel:** `pause` and `resume` are separate processes and must not write `intervals` themselves, because the daemon is the sole writer and a second writer would race the `idx_intervals_one_open` invariant. They talk to the daemon over a `SOCK_STREAM` unix socket in `$XDG_RUNTIME_DIR`, the same directory RF-34 already uses for the lock file, so no new trust boundary is introduced; the daemon checks `SO_PEERCRED` as defence in depth. A socket rather than a watched state file because RF-60 requires distinct exit codes: a file drop cannot report "already paused" or "daemon not running", so `pause` would exit 0 while nothing happened.
...
| **RNF-5** | The daemon opens no network. Only the MCP process speaks, and only over stdio. | Correct, and it stays achievable as written. This is a requirement about **runtime behaviour**, not about the dependency graph: `zbus` links TCP and VSOCK transports unconditionally, but linking transport code is not opening a socket. Materialized in the systemd unit with `RestrictAddressFamilies=AF_UNIX` and `IPAddressDeny=any`, and *(added 2026-09-17)* **verified** by an automated test asserting that every entry in `/proc/self/fd` is a unix socket, a pipe or a regular file after the daemon has been running. That test is what turns RNF-5 from an assertion into a check. |
...
| **RNF-9** *(new)* | Panic isolation in the MCP process: no panic in a handler may bring down the whole connection. | `panic = "unwind"` is the shared release profile selected by D-2; MCP dispatch can isolate handler panics instead of aborting the long-lived process. |
...
No async runtime **of our own** in the daemon: a single `poll(2)` reactor thread over the X11 descriptor, a logind bridge `eventfd`, a signal self-pipe and the control socket. *(Corrected 2026-09-17.)* The earlier wording — "no async runtime in the daemon" — was factually wrong: `zbus::blocking` is a `block_on` facade over the async connection, it never exposes the D-Bus socket descriptor, and it spawns its own executor thread by default (`internal_executor` defaults to `true`). D-Bus is therefore **bridged, not polled**: one owned thread translates D-Bus signals into events on an `eventfd` the reactor can wait on. The daemon runs 3–4 threads, not 1, which is why RNF-1 must be measured as real `RssAnon` rather than argued. `rmcp` uses `tokio`, but only in the MCP process, which lives as long as the conversation does. **Note:** even though tokio is only *used* in MCP mode, being a single binary it is always *linked*; that is the cause of the RNF-3 adjustment.
...
- [ ] RNF-1 to RNF-4 measured with a concrete tool and threshold, with the numeric result in the phase PR, not asserted. *(Clarified 2026-09-17.)* **Which of them gate CI is governed by §15.6, not by this bullet**, which is a summary and was being misread as "all four are enforced": RNF-4 and RNF-3 are hard gates, RNF-1 gates with margin, and **RNF-2 is a documented local benchmark and explicitly not a CI gate**, because virtualized runners are too noisy to measure "zero wakeups". Note also that zero wakeups is provable only for the reactor thread; the bridge threads are measured per-thread and reported honestly.
...
|---|---|---|
| Phase 1 | RF-1 to RF-13, RF-19 (partial: `daemon`, `status`, `today`, `pause`, `resume`, `prune`, `forget`, `completions`), RF-20 to RF-36, RF-47 to RF-54, RF-60 to RF-63, RF-65, RF-66; RNF-1 to RNF-6, RNF-11 | Unit tests, Xvfb+WM integration, properties P1-P3, benchmarks |
...
| Gaps found while planning Phase 1 | RF-65 … RF-66 | logind-unreachable degradation chain, interval clipping |
